...
- `R0` - `R12`
- `R13` (a.k.a. `SP`)
    - Alias to `MSP` if the `SPSEL` bit of the `CONTROL` register is 0
    - Alias to `PSP` if the `SPSEL` bit of the `CONTROL` register is 1
- `R14` (a.k.a. `LR`)
- `xPSR`
- `CONTROL`
...
2. Set `MSP` to the first value in the **Vector Table** (which is the address specified by linker symbol `__StackTop` defined in `stm32l4xx_gcc.ld`, see `startup_stm32l4xx.c`) and align it *downward* to the nearest 8-byte boudary.
...
1. Set `PSP` to the address specified by linker symbol `__StackTop` (defined in `stm32l4xx_gcc.ld`).
...
Upon the entry of the exception, the processor (hardware) will do the following:
1. Push the values of `R0` - `R3`, `R12`, `LR`, `ReturnAddress`[^1] and `xPSR` onto the stack pointed to by `MSP`.
...
[^1]: See *Armv7-M Architecture Reference Manual `ReturnAddress()`* for the definition.
...
4. Set `LR` to `0xFFFFFFFD`[^2] to instruct the processor to enter **Thread Mode** and use `PSP` as `SP` upon exception return[^3].
5. Enable **SysTick interrupt** to perform task switch later on.
[^2]: See *Armv7-M Architecture Reference Manual Table B1-8* for the effects of different `LR` values.
[^3]: Setting `LR` to `0xFFFFFFFD` will cause the hardware to set the `SPSEL` bit of the `CONTROL` register to 1. (*Armv7-M Architecture Reference Manual B1.5.8 Exception return behavior*)

Upon the return of the exception, the processor (hardware) will do the following:
- Enter **Thread Mode** and use `PSP` as `SP`
- Pop the values of `R0` - `R3`, `R12`, `LR`, `ReturnAddress` and `xPSR` of the task from the stack pointed to by `PSP`.
...
1. Push the values of `R0` - `R3`, `R12`, `LR`, `ReturnAddress` and `xPSR` of the current task onto the stack pointed to by `PSP`.
2. Enter **Handler Mode** and use `MSP` as `SP`.
3. ... (See *Armv7-M Architecture Reference Manual B1.5.6 Exception entry behavior*)
...
- Enter **Thread Mode** and use `PSP` as `SP`.
- Pop the values of `R0` - `R3`, `R12`, `LR`, `ReturnAddress` and `xPSR` of the new task from the stack pointed to by `PSP`.
- ... (See *Armv7-M Architecture Reference Manual B1.5.8 Exception return behavior*)
